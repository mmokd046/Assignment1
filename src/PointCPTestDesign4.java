 ...
 * @version July 2000
 */
public class PointCPTestDesign4
{
  //Class methods *****************************************************
  ...
  {
	  coordinateComputation point;

    System.out.println("Design 6 UI Test");
    ...
    if(point instanceof PointCPDesign2) {
        coordinateComputation cartesianPoint = point.convertStorageToCartesian();
        System.out.println(cartesianPoint);
    }
    else {
    	 coordinateComputation cartesianPoint = point.convertStorageToPolar();
         System.out.println(cartesianPoint);
    }
    System.out.println(" ");
    ...
    //Might modify it to make it more user friendly
    double minimumValue = 100;
    double maximumValue = 100000;
    TestPointCPDesign4Performance(minimumValue,maximumValue);
  }
   ...
   *         the user.
   */
  private static coordinateComputation getInput() throws IOException
  {
    ...
    return  generateDesign(coordType, a, b);

  }
  
  private static coordinateComputation generateDesign (char typeCoord, double a, double b) {
	    if (typeCoord == 'C') {
	    	return (new PointCPDesign3(typeCoord, a, b));
	    }
	    else {
	    	return (new PointCPDesign2(typeCoord, a, b));
	    }
  }
  
  private static void TestPointCPDesign4Performance (double minimumValue, double maximumValue) {
	  ...
	  for (int i =0; i<max_iteration;i++) {
		  ...
		  coordinateComputation pointTestA = new PointCPDesign2(coordType, a, b);
		  a = minimumValue + (maximumValue - minimumValue) * rand.nextDouble()  ; 
		  b = minimumValue + (maximumValue - minimumValue) * rand.nextDouble();
		  coordinateComputation pointTestB = generateDesign(coordType, a, b);
